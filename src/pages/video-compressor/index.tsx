  ...
  return (
    <div className="space-y-6 px-6 pb-12 animate-in fade-in duration-300">
      <div className="sticky top-0 z-10 -mx-6 flex flex-col justify-between gap-4 border-b bg-background/95 px-6 py-6 backdrop-blur supports-[backdrop-filter]:bg-background/60 md:flex-row md:items-start">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t("视频处理")}</h1>
          <p className="mt-1 text-muted-foreground">{t("将视频转换为动态 WebP、高压缩比 AV1 或提取音频，支持批量处理。")}</p>
          {page.runtime ? (
            <p className="mt-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <TargetCard checked={page.targets.webp} onToggle={() => page.toggleTarget("webp")} icon={<FileImage className="mb-2 h-8 w-8 opacity-80" />} label={t("WebP 动画")} />
                <TargetCard checked={page.targets.av1} onToggle={() => page.toggleTarget("av1")} icon={<FileVideo className="mb-2 h-8 w-8 opacity-80" />} label={t("AV1 视频")} />
                <TargetCard checked={page.targets.av1_an} onToggle={() => page.toggleTarget("av1_an")} icon={<Scissors className="mb-2 h-8 w-8 opacity-80" />} label={<>AV1<br />({t("无音轨")})</>} />
                <TargetCard checked={page.targets.mp3} onToggle={() => page.toggleTarget("mp3")} icon={<Music className="mb-2 h-8 w-8 opacity-80" />} label={t("提取音频")} />
              </div>
                ...
                <div className="space-y-4 border-t pt-4">
                  <div className="flex flex-col items-start text-left">
                    <div className="flex w-full items-center justify-between">
                      <Label className="text-sm font-medium">{t("hqdn3d 去噪参数设置")}</Label>
                      <div className="flex items-center space-x-2">
                        <Label className="whitespace-nowrap text-xs text-muted-foreground">{t("滑块最大值")}:</Label>
                    ...
                    <span className="mt-1 text-xs font-normal text-muted-foreground">
                      {t("参数全部为 0 时不启用该滤镜。参数详情请参考：")}
                      <button onClick={() => openUrl("https://ffmpeg.org/ffmpeg.html")} className="ml-1 text-primary hover:underline">{t("FFmpeg 官方文档")}</button>
                    </span>
                  </div>
              ...
              <div className="space-y-3 border-t pt-4">
                <Label htmlFor="video-corner-r">{t("圆角裁剪半径 (px)")}</Label>
                <Input id="video-corner-r" type="number" placeholder={t("无圆角 (0)")} value={page.cornerRadius} onChange={(event) => page.setCornerRadius(event.target.value)} min={0} className="max-w-[200px]" />
                <p className="text-xs text-muted-foreground">{t("仅影响 WebP 与 AV1 画面的四周圆角裁剪。")}</p>
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="space-y-5">
              <div className="space-y-3">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <Button variant={page.selectedFile && !page.isFolder ? "default" : "outline"} onClick={() => page.selectSource(false)} disabled={page.busy} className="w-full sm:w-auto">
                    <Video className="h-4 w-4" /> {t("单个视频")}
                  </Button>
                  <Button variant={page.selectedFile && page.isFolder ? "default" : "outline"} onClick={() => page.selectSource(true)} disabled={page.busy} className="w-full sm:w-auto">
                    <FolderOpen className="h-4 w-4" /> {t("视频文件夹")}
                  </Button>
                </div>
                {page.selectedSource ? (
                    ...
                    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)]">
                      <div className="rounded-md border bg-muted/40 p-3">
                        <div className="mb-2 flex items-center justify-between gap-3">
                          <Label className="text-sm font-medium">{t("输入文件")}</Label>
                        ...
                        {page.isFolder ? (
                          <div className="mb-2 flex items-center justify-between gap-2 rounded border bg-background px-2 py-1.5 font-mono text-xs">
                            ...
                            <Button variant="ghost" size="icon-xs" title={t("打开所在位置")} onClick={() => revealItemInDir(page.selectedFile)}>
                        ...
                        {page.inputFiles.length > 0 ? (
                              ...
                              return (
                                <div key={path} className={`flex items-start gap-2 rounded-md border px-2 py-2 transition-colors ${active ? "border-primary/40 bg-primary/5" : "border-transparent bg-background/70 hover:border-border"}`}>
                                  ...
                                  <div className="flex shrink-0 gap-1">
                                    <Button variant="ghost" size="icon-xs" title={t("查看详情")} onClick={() => page.loadVideoDetails(path)}>
                          ...
                          <div className="rounded-md border border-dashed bg-background px-3 py-8 text-center text-sm text-muted-foreground">
                            {page.isFolder ? t("未发现支持的视频文件") : t("尚未选择视频文件")}
                          </div>
                        )}
                      </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="secondary" onClick={page.selectOutDir} disabled={page.busy}>{t("更改")}</Button>
                  {page.outputDir ? (
                    <>
                      <Button variant="ghost" onClick={() => page.setOutputDir("")} disabled={page.busy}>{t("清除")}</Button>
          ...
          {page.error ? (
            <div className="flex gap-3 rounded-lg border border-destructive/20 bg-destructive/10 p-4 text-destructive animate-in slide-in-from-top-2">
              ...
              <div className="text-sm">
                <strong className="mb-1 block font-semibold">{t("执行失败")}</strong>
