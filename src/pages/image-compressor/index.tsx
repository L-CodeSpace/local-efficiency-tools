    ...
    <div className="space-y-6 px-6 pb-12 animate-in fade-in duration-300">
      <div className="sticky top-0 z-10 -mx-6 flex flex-col justify-between gap-4 border-b bg-background/95 px-6 py-6 backdrop-blur supports-[backdrop-filter]:bg-background/60 md:flex-row md:items-start">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">将常见图片格式及 RAW/HEIC 转换为 WebP 或 AVIF，支持单文件与整个文件夹</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>调整图片画质、特效等参数</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Label>导出质量 ({page.quality})</Label>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-3">
              <Label htmlFor="corner-r">圆角半径 (px)</Label>
              <Input
                id="corner-r"
                type="number"
                placeholder="无圆角 (0)"
                  ...
                  {page.selectedSource.type === "folder" ? (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Button variant="ghost" size="icon-xs" title="打开所在位置" onClick={() => revealItemInDir(page.selectedSource!.type === "folder" ? page.selectedSource!.path : page.selectedSource!.paths[0])}>
                      ...
                      {page.folderFilesPreview && page.folderFilesPreview.length === 0 ? (
                        <div className="mt-2 border-t border-muted-foreground/20 pt-2 text-xs text-muted-foreground">未发现支持的图片文件</div>
                      ) : null}
                    </div>
                  ) : (
          ...
          <div className="space-y-3 border-t pt-4">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="secondary" onClick={page.selectOutDir} disabled={page.busy} title="更改输出目录">
                <FolderOpen className="h-4 w-4" />
              </Button>
              {page.outputDir ? (
          ...
          <div className="pt-2 text-xs text-muted-foreground">
            支持格式：JPG · PNG · GIF · BMP · TIFF · HEIC · AVIF · RAW 等 → <strong className="text-primary">WebP / AVIF</strong>
          </div>
        </CardContent>
      </Card>
      ...
      {page.error ? (
        <div className="flex gap-3 rounded-lg border border-destructive/20 bg-destructive/10 p-4 text-destructive">
          ...
          <div className="text-sm">
            <strong className="mb-1 block font-semibold">操作失败</strong>
