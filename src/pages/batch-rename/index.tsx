  ...
  return (
    <div className="flex h-full flex-col space-y-6 px-6 pb-12 animate-in fade-in duration-300">
      <div className="sticky top-0 z-10 -mx-6 flex shrink-0 flex-col justify-between gap-4 border-b bg-background/95 px-6 py-6 backdrop-blur supports-[backdrop-filter]:bg-background/60 md:flex-row md:items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">批量重命名</h1>
          <p className="mt-1 text-muted-foreground">使用正则表达式高级批量重命名文件</p>
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-6">
        <div className="shrink-0 grid-cols-1 gap-6 md:grid-cols-3">
          <Card className="rounded-xl shadow-sm">
            <CardHeader className="p-5 pb-0">
              <CardTitle className="flex items-center gap-2 text-base font-semibold normal-case tracking-normal">
                <Settings2 className="h-4 w-4" /> 目标设置
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 p-5">
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">当前目录</Label>
                <div className="group flex items-center justify-between gap-2 truncate rounded-md bg-muted/50 p-2 font-mono text-sm" title={page.root || "未选择"}>
                  <span className="truncate">{page.root || "未选择"}</span>
                  {page.root ? (
                    <Button variant="ghost" size="icon-xs" title="打开所在位置" onClick={() => revealItemInDir(page.root)}>
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">递归深度</Label>
              ...
              <div className="space-y-3 border-t pt-3">
                <label className="flex items-center space-x-2">
                  ...
                  <span className="text-xs font-medium">自动解决冲突 (添加数字后缀)</span>
                </label>
                {page.autoResolveCollision ? (
                  <div className="flex items-center space-x-2 pl-6">
                    <Label className="whitespace-nowrap text-xs text-muted-foreground">起始序号</Label>
                ...
                <label className="flex items-center space-x-2">
                  ...
                  <span className="text-xs font-medium">保留原文件后缀名</span>
                </label>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-xl shadow-sm">
            <CardHeader className="p-5 pb-0">
              <CardTitle className="flex items-center gap-2 text-base font-semibold normal-case tracking-normal">
                <Type className="h-4 w-4" /> 预设规则
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 p-5">
              <div className="grid grid-cols-1 gap-2">
                <Button variant="secondary" size="sm" className="justify-start" onClick={() => page.applyPreset("(.+)", "$1-副本")}>添加后缀: name -&gt; name-副本</Button>
                <Button variant="secondary" size="sm" className="justify-start" onClick={() => page.applyPreset("(.+)\\.(.+)$", "$1-$INDEX.$2")}>序列命名: name-1, name-2</Button>
                <Button variant="secondary" size="sm" className="justify-start" onClick={() => page.applyPreset("oldName", "newName")}>简单替换: oldName -&gt; newName</Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-xl shadow-sm">
            <CardHeader className="p-5 pb-0">
              <CardTitle className="flex items-center gap-2 text-base font-semibold normal-case tracking-normal">
                <Search className="h-4 w-4" /> 正则表达式规则
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 p-5">
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">查找 (Regex)</Label>
                <Input placeholder="例如: (.*)\\.png$" value={page.pattern} onChange={(event) => page.setPattern(event.target.value)} className="font-mono text-sm" />
              </div>
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">替换为</Label>
                <Input placeholder="例如: $1-compress.png" value={page.replacement} onChange={(event) => page.setReplacement(event.target.value)} className="font-mono text-sm" />
                <p className="mt-1 text-[10px] text-muted-foreground">支持 $1, $2 分组，使用 $INDEX 插入自增序号</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex min-h-[400px] flex-1 flex-col overflow-hidden rounded-xl border bg-card text-card-foreground shadow-sm">
          <div className="flex shrink-0 items-center justify-between border-b bg-muted/20 p-4">
            <h2 className="flex items-center gap-2 font-semibold">
              <FileText className="h-4 w-4" /> 预览与执行
            </h2>
            <div className="flex items-center gap-3">
              <span className="text-sm text-muted-foreground">已选: {selectedCount} / 变更: {changedCount}</span>
          ...
          {page.busy ? (
            <div className="shrink-0 border-b bg-primary/5 p-4">
              <div className="mb-2 flex items-center justify-between text-xs"><span>处理中</span><span>...</span></div>
              ...
              <Table>
                <TableHeader className="sticky top-0 z-10 bg-card shadow-sm">
                  <TableRow>
                    ...
                    <TableHead className="w-[45%]">新名称</TableHead>
                    <TableHead className="w-[45%]">原名称</TableHead>
                    ...
                    return (
                      <TableRow key={item.originalPath} className={item.collision ? "bg-destructive/10 hover:bg-destructive/15" : changed ? "bg-primary/5 hover:bg-primary/10" : ""}>
                        ...
                        <TableCell className="break-all font-mono text-xs">
                          ...
                          {item.collision ? <div className="mt-1 flex items-center gap-1 text-[10px] text-destructive"><Trash2 className="h-3 w-3" /> 命名冲突</div> : null}
                        </TableCell>
                        <TableCell className="break-all font-mono text-xs opacity-70">{relativePath(page.root, item.originalPath)}</TableCell>
                        <TableCell className="text-center text-xs">
                          {changed && !item.collision ? <Badge variant="outline">待处理</Badge> : null}
                          {item.autoResolved ? <Badge variant="secondary" className="mt-1 scale-90 whitespace-nowrap text-[10px]">已追加序号</Badge> : null}
